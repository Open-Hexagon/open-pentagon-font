...
for codepoint, name in bs_cmap.items():
    ...
    transform = Identity.scale(bs_scale, bs_scale).translate(
        (FONT_EM - BS_ICON_SIZE) / 2 / bs_scale,
    ...
    font_lut[BS_PREFIX + bs_ord2codename[codepoint]] = ord2utf8str(
        next_private_codepoint
    )
...
for codepoint, name in pf_cmap.items():
    glyph_name = PF_PREFIX + (pf_ord2codename.get(codepoint) or name)
    if glyph_name in font:
        continue
